...
function refreshCache()
	...
	--
	local period = luup.variable_get(MPCCTRL_SERVICE, "refreshperiod", PARENT_DEVICE)
	...
	local lurl = luup.variable_get(MPCCTRL_SERVICE, "rssURL", PARENT_DEVICE)
	local lusername = luup.variable_get(MPCCTRL_SERVICE, "userName", PARENT_DEVICE)
	local lpassword = luup.variable_get(MPCCTRL_SERVICE, "Password", PARENT_DEVICE) 
	local lselected = luup.variable_get(MPCCTRL_SERVICE, "selectedItem", PARENT_DEVICE) 
	...
	if (title) then
	  ...
	  changed = setIfChanged(MPCCTRL_SERVICE, "rssTitle", title,
						     PARENT_DEVICE, changed)
							 
	  changed = setIfChanged(MPCCTRL_SERVICE, "rssDescription", descr,
						     PARENT_DEVICE, changed)	
							 
	  changed = setIfChanged(MPCCTRL_SERVICE, "rssContent", content,
						     PARENT_DEVICE, changed)	

	  changed = setIfChanged(MPCCTRL_SERVICE, "rssLink", link,
						     PARENT_DEVICE, changed)

	  changed = setIfChanged(MPCCTRL_SERVICE, "itemsCount", lcount,
						     PARENT_DEVICE, changed)								 
...
function startupDeferred()
	local url = luup.variable_get(MPCCTRL_SERVICE, "rssURL", PARENT_DEVICE)
	if (url == nil or url == "" ) then
		--
		-- Set the variable so that it appears in the Device/Advanced list
		--
		luup.variable_set(MPCCTRL_SERVICE, "rssURL", "", PARENT_DEVICE)
		luup.variable_set(MPCCTRL_SERVICE, "userName", "", PARENT_DEVICE)
		luup.variable_set(MPCCTRL_SERVICE, "Password", "", PARENT_DEVICE)
...
function initialize(parentDevice)
	...
	--
	if (luup.variable_get(MPCCTRL_SERVICE, "rssURL", parentDevice) == nil) then
		luup.variable_set(MPCCTRL_SERVICE, "rssURL", "", parentDevice)
	end

	if (luup.variable_get(MPCCTRL_SERVICE, "userName", parentDevice) == nil) then
		luup.variable_set(MPCCTRL_SERVICE, "userName", "", parentDevice)
	end	
	if (luup.variable_get(MPCCTRL_SERVICE, "Password", parentDevice) == nil) then
		luup.variable_set(MPCCTRL_SERVICE, "Password", "", parentDevice)
	end	
	if (luup.variable_get(MPCCTRL_SERVICE, "refreshperiod", parentDevice) == nil) then
		luup.variable_set(MPCCTRL_SERVICE, "refreshperiod", "", parentDevice)
	end		
	if (luup.variable_get(MPCCTRL_SERVICE, "weatherItem", parentDevice) == nil) then
		luup.variable_set(MPCCTRL_SERVICE, "weatherItem", "0", parentDevice)
	end		
	if (luup.variable_get(MPCCTRL_SERVICE, "selectedItem", parentDevice) == nil) then
		luup.variable_set(MPCCTRL_SERVICE, "selectedItem", "1", parentDevice)
	end	
...
function test()	
	...
	luup.log(link)
	luup.log(lcount)
	local weather = luup.variable_get(MPCCTRL_SERVICE, "weatherItem", 188)
	if weather == "1" then
	  luup.log("je 1")
	else
	  luup.log("je 0")
	end
	luup.variable_set(MPCCTRL_SERVICE, "weatherItem", "1", 188)
	weather = luup.variable_get(MPCCTRL_SERVICE, "weatherItem", 188)
	if weather == "1" then
	  luup.log("je 1")
	else
	  luup.log("je 0")
